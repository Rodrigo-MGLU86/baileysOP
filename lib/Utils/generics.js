...
const printQRIfNecessaryListener = (ev, logger) => {
    ...
    ev.on('connection.update', async ({ qr }) => {
        if (qr) {
            const now = Date.now()
            if (qr === lastPrintedQr && (now - lastPrintedAt) < 90_000) {
                return
            }
            lastPrintedQr = qr
            lastPrintedAt = now
            ...
            try {
                const branding = await branding_1.getQrBranding()
                branding_1.writeBrandingLines(branding.headerLines, logger)
                QR.generate(qr, { small: true })
                branding_1.writeBrandingLines(branding.footerLines, logger)
            }
            catch (_error) {
                QR.generate(qr, { small: true })
            }
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/neelify/baileys/master/src/Defaults/baileys-version.json'
    try {
